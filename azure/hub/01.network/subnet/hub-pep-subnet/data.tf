...
data "terraform_remote_state" "hub_nsg" {
  ...
  config = {
    ...
    storage_account_name = var.hub_backend_storage_account_name
    container_name       = var.hub_backend_container_name
    key                  = "azure/dev/hub/01.network/security-group/network-security-group/hub-pep/terraform.tfstate"
  }
}
...
data "terraform_remote_state" "sg_hub_keyvault_clients_asg" {
  ...
  config = {
    ...
    storage_account_name = var.hub_backend_storage_account_name
    container_name       = var.hub_backend_container_name
    key                  = "azure/dev/hub/01.network/security-group/application-security-group/keyvault-clients/terraform.tfstate"
  }
}
...
data "terraform_remote_state" "sg_hub_vm_allowed_clients_asg" {
  ...
  config = {
    ...
    storage_account_name = var.hub_backend_storage_account_name
    container_name       = var.hub_backend_container_name
    key                  = "azure/dev/hub/01.network/security-group/application-security-group/vm-allowed-clients/terraform.tfstate"
  }
}
