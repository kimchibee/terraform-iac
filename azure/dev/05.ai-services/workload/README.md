...
|------|-----------|-----------------------------|
| 1. Backend 초기화 | state 키 `azure/dev/05.ai-services/workload/terraform.tfstate`. | `backend.hcl` |
| 2. 변수 로드 | `terraform.tfvars` → `variables.tf`. | project_name, openai_sku, openai_deployments, backend_* 등 |
| 3. **remote_state: network** | Hub/Spoke VNet·서브넷·Private DNS Zone ID. OpenAI·PE·ML Workspace 배치용. | Hub: `azure/dev/01.network/vnet/hub-vnet/terraform.tfstate`, Spoke: `azure/dev/01.network/vnet/spoke-vnet/terraform.tfstate`. **선행:** Hub·Spoke network apply 완료. |
| 4. **remote_state: storage** | Key Vault, Storage 계정 ID 등. | `azure/dev/02.storage/monitoring/terraform.tfstate` |
| 5. **remote_state: shared_services** | Log Analytics 등. | `azure/dev/03.shared-services/shared/terraform.tfstate` |
