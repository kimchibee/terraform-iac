...
State 1개, **이 리프 `main.tf`**에서만 apply 합니다. `monitoring-storage`는 terraform-modules Git 모듈을 직접 참조합니다.

> Strict AVM Foundation 기준에서 이 리프는 `monitoring-storage`(AVM+azurerm hybrid) 의존 때문에 **Deferred** 입니다.
> 신규 리프는 이 패턴을 복제하지 않고, 후속 파동에서 `storage-account` + `key-vault` + `private-endpoint` 조합으로 전환합니다.
...
| 파일 | 역할 | 주로 수정하는 내용 |
|------|------|-------------------|
| `main.tf` | `terraform_remote_state`(network, compute), `module "storage"` 호출 | 모듈 블록 추가 시 remote_state 출력을 인자로 전달 |
| `variables.tf` | 루트 변수 선언 | 공통 변수 추가 시 |
| `terraform.tfvars` | 실제 값 | `hub_subscription_id`, `backend_*`, (선택) `monitoring_vm_identity_principal_id` — compute를 먼저 배포하지 않았을 때 MI principal 수동 지정 |
| `backend.tf` / `backend.hcl` | 원격 state | Bootstrap 후 생성된 `backend.hcl`과 정합 |
| `provider.tf` | Hub 구독 provider | 거의 고정 |
| `outputs.tf` | Key Vault ID, Storage ID 등 | rbac·다른 스택 연동용 |

### 하위 디렉터리

| 디렉터리 | 역할 | 신규/변경 시 |
|----------|------|----------------|
| `monitoring-storage/` | Key Vault·모니터링 Storage·PE (Git 모듈 래퍼) | **폴더 복사 후** 해당 폴더 `variables.tf`에서 `key_vault_suffix`, `enable_key_vault`, `enable_monitoring_vm` 등 수정 |

### 신규 리소스 생성 시 변수(의미)

| 작업 | 수정 위치 | 의미 |
|------|-----------|------|
| **Monitoring Storage 세트 추가** | 새 `monitoring-storage-xx/variables.tf` | `key_vault_suffix`: 이름 구분. `enable_key_vault` / `enable_monitoring_vm`: 해당 스택에서 만들 리소스 선택 |
| **루트에서 연결** | 루트 `main.tf` | `resource_group_name`, `pep_subnet_id` 등은 **network state 출력**에서만 가져옴(직접 ID 하드코딩 지양) |
| **MI 연동** | `terraform.tfvars` 또는 compute 선배포 | `monitoring_vm_identity_principal_id`: Storage/Key Vault 접근 주체로 쓸 Monitoring VM의 MI Object ID(비우면 compute state에서 조회 시도) |
...
|------|-----------|-----------------------------|
| 1. Backend 초기화 | state 키 `azure/dev/02.storage/monitoring/terraform.tfstate`. | `backend.hcl` |
| 2. 변수 로드 | `terraform.tfvars` → `variables.tf`. | project_name, backend_* 등. Key Vault/모니터링 옵션은 monitoring-storage 폴더 기본값. |
| 3. **remote_state: network** | Network 스택 state 조회. Hub RG 이름, 서브넷 ID, Private DNS Zone ID 등 획득. | `azure/dev/01.network/vnet/hub-vnet/terraform.tfstate`. **선행:** Hub network apply 완료. |
| 4. **remote_state: compute** (선택) | Monitoring VM Identity 사용 시 Compute 스택 state 조회. principal_id 획득. | `azure/dev/06.compute/linux-monitoring-vm/terraform.tfstate`. 없으면 `var.monitoring_vm_identity_principal_id` 사용 가능. |
