...
|------|-----------|-----------------------------|
| 1. Backend 초기화 | state 키 `azure/dev/connectivity/terraform.tfstate`. | `backend.hcl` |
...
| 5. **remote_state: shared_services** | Log Analytics 등(진단 설정 대상으로 사용 시). | `azure/dev/shared-services/terraform.tfstate` |
