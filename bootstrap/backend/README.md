...
```bash
cd /Users/chi-sungkim/azure_ai/terraform-iac/bootstrap/backend
...
```bash
cd /Users/chi-sungkim/azure_ai/terraform-iac
./scripts/generate-backend-hcl.sh
```

- **실행 시점:** Bootstrap **apply 완료 후**, `bootstrap/backend/terraform.tfvars`가 준비된 상태에서 실행합니다.
- **생성 위치:** `azure/dev/network`, `azure/dev/storage`, `azure/dev/shared-services`, `azure/dev/apim`, `azure/dev/ai-services`, `azure/dev/compute`, `azure/dev/rbac`, `azure/dev/connectivity` 디렉터리에 각각 `backend.hcl`이 생성됩니다.
