...
| 파일 | 역할 | 주로 수정하는 내용 |
|------|------|-------------------|
| `main.tf` | RG, Storage Account, Container, (선택) PE | Backend 인프라 정의. **일반적으로 모듈 없이** azurerm 리소스 직접 |
| `variables.tf` | 변수 선언 | `resource_group_name`, `storage_account_name` 등 |
| `terraform.tfvars` | 실제 배포 값 | **반드시** 전역 유일한 `storage_account_name` 등 입력 |
| `terraform.tfvars.example` | 입력 예시 | 복사하여 `terraform.tfvars` 생성 |
| `outputs.tf` | Storage 이름·RG 등 출력 | `generate-backend-hcl.sh` 등에서 참조 가능 |
| **원격 backend** | 이 스택에서는 **사용하지 않음** | `terraform init`만 실행(`backend.hcl` 없음) |
...
| 작업 | 수정 위치 | 의미 |
|------|-----------|------|
| **최초 배포** | `terraform.tfvars` | `resource_group_name`, `storage_account_name`(전역 유일), `container_name`, `location`, (선택) `subnet_id` for PE |
| **이름 변경** | 주의 | Storage Account 이름 변경은 **기존 state와 충돌**할 수 있음. 마이그레이션 없이 변경하지 않는 것이 안전 |
| **리소스 추가** | `main.tf` | 예: 추가 컨테이너 — 드물게 수행 |
...
- **생성 위치:** `scripts/generate-backend-hcl.sh`의 **`INFRA_LEAVES`**, **`COMPUTE_SUBDIRS`**, **`RBAC_IDENTITY_LEAVES`**, **`CONNECTIVITY_LEAVES`** 에 나열된 **리프 디렉터리** 각각에 `backend.hcl`이 생성됩니다. (`01.network/resource-group/hub-rg`, `01.network/application-security-group/keyvault-clients`, `01.network/network-security-group/keyvault-standalone`, `01.network/vnet/hub-vnet`, `01.network/subnet/hub-pep-subnet`, `01.network/vnet/spoke-vnet`, `01.network/route/hub-route-default`, `01.network/route/spoke-route-default`, `02.storage/monitoring`, …)
